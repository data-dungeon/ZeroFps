...
void MistClient::SetupGUI()
{
	...
	g_kMistClient.InitGui(m_pkScript, "morpheus10", "data/script/gui/defskins.lua", NULL, false, true); 
   ...
   font->Create(szFontData, m_pkTexMan->Load(szFontTex, 0));
	g_kMistClient.m_pkGui->GetResMan()->Add("listboxfont", font);

   // load startup screen 
   if(!g_kMistClient.m_bSkipLoginScreen)
   {
      g_kMistClient.LoadGuiFromScript("data/script/gui/ml_start.lua");
