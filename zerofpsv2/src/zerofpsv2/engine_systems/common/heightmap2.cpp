	...
	{	
		...
		{	
			...
			{				
				...
				newlevel.kMaskTexCor.push_back(kMaskTexCor);					
				...
				kMaskTexCor.x = float(x) / float(m_iWidth);
				kMaskTexCor.y = (float(y) / float(m_iHeight)) + float(iStep)/float(m_iHeight);				
				newlevel.kTexCor.push_back(kTexCor);					
