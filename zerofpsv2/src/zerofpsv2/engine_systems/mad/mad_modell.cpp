...
void Mad_Modell::Draw_All_RenderP()
{
	Mad_Core* pkCore = (Mad_Core*)(kMadHandle.GetResourcePtr()); 
	...
	for(int iM = 0; iM <iNumOfMesh; iM++) 
	{
		...
		for(int iSubM = 0; iSubM < iNumOfSubMesh; iSubM++) 
		{
			...
			//grow package list if needed
			if(m_kRenderPackage.size() < iRenderPackage)
				m_kRenderPackage.resize(iRenderPackage);
		
			RenderPackage& kRP = m_kRenderPackage[iRenderPackage-1];
		
			SelectSubMesh(iSubM);
			
			kRP.m_kMeshData.m_kDataPointers.clear();
			...
			{
				kRP.m_kMeshData.m_kDataPointers.push_back(DataPointer(BONEMATRIX_POINTER, kFinalMatrixTransform ));
				kRP.m_kMeshData.m_kDataPointers.push_back(DataPointer(BONEINDEX_POINTER, &m_pkRawMesh->akBoneConnections[0],GetNumVertices()));
			}			
									
			kRP.m_kMeshData.m_kDataPointers.push_back(DataPointer(VERTEX_POINTER, GetVerticesPtr(),GetNumVertices()));
			kRP.m_kMeshData.m_kDataPointers.push_back(DataPointer(TEXTURE_POINTER0, GetTextureCooPtr()));
			kRP.m_kMeshData.m_kDataPointers.push_back(DataPointer(NORMAL_POINTER, GetNormalsPtr()));

			kRP.m_kMeshData.m_kDataPointers.push_back(DataPointer(INDEX_POINTER, GetFacesPtr()));
			kRP.m_kMeshData.m_iNrOfDataElements = GetNumFaces() * 3;
			kRP.m_kMeshData.m_ePolygonMode = TRIANGLES_MODE;					 

			ZFResourceHandle* pkRes = m_pkMesh->GetLODMesh(iLodIndex)->GetTextureHandle(m_pkSubMesh->iTextureIndex);
			ZMaterial* pkMaterial = (ZMaterial*)(pkRes->GetResourcePtr());		
			kRP.m_pkMaterial = pkMaterial; 			
