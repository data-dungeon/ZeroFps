...
bool GLGuiRender::StartRender(bool bClear)
{
	...
	glScissor  ( 0, 0,	m_iScreenWidth, m_iScreenHeight ); // lade till 22 okt 2004 f� att kunna rendera camera i f�ster	
	glViewport ( 0, 0,	m_iScreenWidth, m_iScreenHeight  );	// lade till 22 okt 2004 f� att kunna rendera camera i f�ster	
...
{	
	...
	if(m_pkSkin->m_pkZIFAnimation->Update(bRebuildTexture))
	{
		m_pkSkin->m_iBkTexID = m_pkTextureManger->CreateTextureFromRGB(
			...
			TextureManager::BGR, bRebuildTexture, TextureManager::RGB);
	}
}
									 ...
									 int& rows_printed) 
{
	...
	glBegin(GL_QUADS);	 
		...
		while(offset < characters_totalt) // antal ord
		{
			...
			{
				// R�na inte in sista tecknets l�gd om det � ett space.
				// och om l�gden p�ordet �erskrider den till�na l�gden.
							...
							int offset, int length)
{
	...
	for(i=offset; i<offset+length; i++)
	{
		...
		if(i == m_iCursorPos && !(m_iCursorPos == 0 && strlen(szWord) <= 1))
		{
			...
			iCursorX -= 2;	// minska mark�ens xpos ytterligare 2 pixlar.
								// som en kompensation f� tecknets egen storlek.
