...
bool ZGui::UnregisterWindow(ZGuiWnd* pkWindow)
{
	...
	// Ta bort f�stret i listan hos managern
	m_pkResManager->RemoveWnd(string(pkWindow->m_szName));

	// Ta f�st bort eventuellt main window
	...
	if(itWnd != m_pkWindows.end())
	{
		...
		// Ta bort alla barn f�ster som tillh� detta f�ster.
		// Men bara om det � ett (main) f�ster. I annat fall
		// sk�er kontrollerna det sj�va.
						 ...
						 callback cb,bool bSetAsActive)
{
	...
	// Ett main window skall inte ha n�on parent!
...
{	
   if(pkWnd == NULL)
	...
	// Hitta det f�ster som tidigare hade fokus och 
	// ta bort fokuset fr� denna.
	...
	// Kolla f�st om detta f�ster kan ha keyboard fokus.
	if(!pkWnd->GetWindowFlag(WF_CANHAVEFOCUS) || pkWnd == NULL)
	{
		// F�s� s�ta fokus p�dess root parent ist�let.
...
{
	// Kolla f�st om vi skall k�a ett keycommand...
	...
	{		
		// Skicka ett Command medelande till valt f�ster.
...
{	
	...
	// Finns det redan ett f�ster med det namnet och detta f�ster
	// � inte det f�ster som skall d�as om?
	if(pkExistingWnd != NULL && pkWndToRename != pkExistingWnd) 
	{
		return false; // det finns redan ett f�ster med detta namn.
	}

	// Finns det redan ett f�ster med det namnet och detta f�ster
	// � det f�ster som skall d�as om?
	if(pkExistingWnd != NULL && pkWndToRename == pkExistingWnd) 
	{
		if(strcmp(pkExistingWnd->GetName(), pkWndToRename->GetName()) == 0) 
			return true; // namnet har inte �drats
	}

	if(pkExistingWnd == NULL)
	{
		// Radera (ej deallokera) f�stret i guimanagern.
		if(m_pkResManager->RemoveWnd(pkWndToRename->GetName()) == false)
			return false;

		// L�g till f�stret p�nytt, fast med ett annat namn.
...
// Br� funktionen som man kr�s av att titta i... Gr�funktionen nummer ett.
                         ...
                         float fGameTime)
{
	...
	//////////////////////////////////////////////////////////////////////////////
	// Pgr av att musmark�en som anv�ds i DM inte har sin klickpunkt i �re
	// v�stra h�net utan 2 pixlar l�gre ner, kompenserar vi f� det h�.
	//////////////////////////////////////////////////////////////////////////////
	x+=2; y+=2;
	m_iMouseX = x; m_iMouseY = y;
	//////////////////////////////////////////////////////////////////////////////
	...
	if(m_pkCapturedWindow == NULL)
	{
		...
		{ 
			...
			if((wnd = FindMainWnd(x,y)) != NULL) //if(wnd = FindMainWnd(x,y) ) -- �dring 040701
			{
	...
	else
	{
      ...
      if(bLeftButtonDown && m_bLeftButtonDown==false) // annars k�s den alltid
      {
	...
	// en kontroll � captured och ej � ett f�ster.
	...
	// Har v�ster musknapp klickats (men inte sl�pt)
...
//				if( (test=m_pkActiveMainWin->pkWnd->Find(x,y))) //!ClickedWndAlphaTex(x,y, ZGuiWnd::m_pkWndClicked))
	...
	// � v�ster musknapp nertryckt?
	if( bLeftButtonDown == true && ZGuiWnd::m_pkWndClicked)
	{
		// Skall f�stret flyttas?
	...
	// Har v�ster musknapp sl�ts (men inte klickats)?
	if(bLeftReleased || bRightReleased)
	{
		m_bHandledMouse = false;

		// Byt tillbaks till button_up skin om musknappen har sl�pts (fast inte p�knappen).
		if(ZGuiWnd::m_pkWndClicked)
		...
		{
			// Informera f�stret innan att det har tappat fokus.
			...
			// � mark�en fortfarande innanf� f�strets gr�ser?
			if(ZGuiWnd::m_pkWndClicked->GetScreenRect().Inside(x,y))
			{
				...
				if(bLeftReleased || (pkFocusWindow->m_bAcceptRightClicks && bRightReleased))
				{
					...
					if( pkParent)
					{
						...
						//		typeid(ZGuiWnd::m_pkWndClicked)!=typeid(ZGuiCheckbox)) // tillf�lig ful l�ning f� att listboxitems inte skall generera COMMAND messages..
						//{
							ZGuiWnd* pkMainWnd = pkParent; //m_pkActiveMainWin->pkWnd; // Lade till 9 nov 2004 f� att controllers p�en tabctrl inte f� msg annars.

							// Menyalternativ har sitt egen f��derf�ster.
							if(pkParent && typeid(*pkParent)==typeid(ZGuiMenu))
								pkMainWnd = pkParent;

							// Har man klickat p�en checkbox via en label s�� klickf�stret checkboxen.
							...
							pkParams[1] = (pkFocusWindow->m_bAcceptRightClicks && bRightReleased); // h�er musknapp har triggat knapp kommandot
	...
	// � v�ster musknapp inte nertryckt?
	if(m_bLeftButtonDown == false && bLeftButtonDown == false)
	{
		// � det samma f�stret under musmark�en som innan?
