...
--	SISetHeartRate(SIGetSelfID(),4);
	...
	AddItem(SIGetSelfID(), "data/script/objects/dm/t_rifle.lua", 1); -- snuten skall naturligvis ha en picka i början.
...
function FirstRun()
	...
	SetEntityVar(SIGetSelfID(), "CreateCounter", 0)
...
function HeartBeat()
	...
	-----------------------------------------------------------------------
	-- Vänta på att alla objekt skall ha skapats så att vi kan ta
	-- reda på en lista över alla civila
	-----------------------------------------------------------------------
	...
	if HelpingCharacterID > 0 then
		...
		if range > 10 then -- Inte framme hos den som ropar?
			...
			local enemy = GetDMCharacterClosest(SIGetSelfID())
			agent_pos = GetEntityPos(enemy)
			police_pos = GetEntityPos(SIGetSelfID())
			range = GetRangeBetween(agent_pos, police_pos)

			if range < 10 then
				SetEntityVar(SIGetSelfID(), "g_PoliceEnemyID", enemy)	-- Polisen kan se brottslingen
			else
				SetEntityVar(SIGetSelfID(), "g_PoliceEnemyID", -1)	-- Polisen kan EJ se brottslingen
			end

		end
	end

	local enemy = GetEntityVar(SIGetSelfID(), "g_PoliceEnemyID")

	if ( IsDead(enemy) == 1) then
		SetState(SIGetSelfID(),0) -- Get Idle
	end

	if GetState(SIGetSelfID()) == 4 then -- aggresive
...
function OnTakingDmgFrom(EnemyShooting)
	...
	SetEntityVar(SIGetSelfID(), "g_PoliceEnemyCharacterID", EnemyShooting)
end
