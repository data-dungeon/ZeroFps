...
void Core::Load(const char* MadFileName)
{
	...
	for(i=0; i<kHead.iNumOfFrames; i++)
	{
		...
		pakFrames[i].pNormal = new MadVertex [kHead.iNumOfVertex];
		fread(pakFrames[i].pNormal,sizeof(MadVertex),kHead.iNumOfVertex,fp);
...
//	glDisable(GL_LIGHTING);
	...
	glNormalPointer(GL_FLOAT,0, pakFrames[iActiveFrame].pNormal);
